...
Συνεπώς, η διαφορά προκύπτει από την αναλογία μεταξύ των εντολών που στέλνονται για εκτέλεση και των πραγματικών εργασιών που απαιτούνται για να ολοκληρωθούν αυτές οι εντολές στο εσωτερικό της επεξεργαστικής μονάδας.
...
Με βάση το πρώτο μέρος, για τα πρότυπα  benchmarks του ερωτήματος 2 & 1 από το πρώτο βήμα (και βάσει διαγραμάτων) επιλέχθηκαν οι ακόλουθες τιμές:
...
Στην ουσία, επειδή ο χρόνος εκτέλεσης του κάθε benchmark είναι μεγάλος, δεν ήθελα να πάρω μεγάλο δείγμα. Οπότε στράφηκα σε 5-6 επαναλήψεις για κάθε
benchmark. Στόχος, ήταν να δω που θα ελαττωθεί το CPI. Στις περιπτώσεις specsjeng & speclibm, δεν χρειάστηκε να μεταβώ σε 6η δοκιμή. Συνοπτικά, βλέποντας 
τα γραφήματα του ερωτήματος 2 και έχοντας τα δεδομένα του ερωτήματος 1, όσον αφορά L1, L2 & Associativity της κάθε cache ήξερα αν θα "παίξω" με L1d, L1i, L2 ή 
τα Associativity αντίστοιχα, όπως και δεν μπορούσα να υπέρβω 256KB ως συνολικό μέγεθος L1 cache  και τα 4ΜΒ ως συνολικό μέγεθος L2 cache.
...
Το L2 βλέπουμε πως είναι περισσότερα kb,από L1 οπότε έπρεπε να μειωθεί διαλέξαμε το 32 γιατί είναι η οικονομικότερη παράμετρος, οπότε 1/32. Το 96 στο w3 προέκυψε, καθώς αλλαγή έχουμε στο L1, οι φθηνότερες παράμετροι είναι 64 & 32 του L1 και αν αθροιστούν βγαίνει το 96. Αν αθροίζαμε και του L2, το κόστος θα αυξανόταν αρκετά.
...
* specbzip: Καλύτερη επιλογή βρίσκουμε στο specbzip_0
Το κόστος υπολογίσθηκε 960 και το Score ισούται με 1636,932 . Όπως είχαμε αναφέρει και στο βήμα 2, η απόδοση δεν αλλάζει σημαντικά επομένως για αυτό το benchmark μπορούμε να χρησιμοποιήσουμε όσο μικρότερες και φθηνότερες παραμέτρους.

* specmcf: Καλύτερη επιλογή βρίσκουμε στο speccmcf_1
Το κόστος υπολογίσθηκε 1376 και το Score ισούται με 1581,551 . Εδώ θα χρησιμοποιήσουμε L1_icache = 64kB ή και μεγαλύτερη, και τις υπόλοιπες παραμέτρους θα τις διατηρήσουμε όσο μικρότερες και φθηνότερες γίνεται.

* spechmmer: Καλύτερη επιλογή βρίσκουμε στο spechmmer_0
Το κόστος υπολογίσθηκε 1344 και το Score ισούται με 1901,368. Εδώ μπορούμε να χρησιμοποιήσουμε όσο μικρότερες και φθηνότερες παραμέτρους γίνεται. Δεδομένου της συνάρτησης κόστους που χρησιμοποιήσαμε παρατηρούμε ότι δεν αυξάνεται ιδαίτερα το Score με αύξηση της L1_dcache. Δεδομένου ότι μειώνει σημαντικά το miss rate της θα μπορούσαμε να τη διατηρήσουμε υψηλά.


* specsjeng: Καλύτερη επιλογή βρίσκουμε στο specjeng_4
Το κόστος υπολογίσθηκε 1760 και το Score ισούται με 11971,47. Εδώ διατηρούμε το μέγεθος της cacheline στα 128B ή και περισσότερο και τις υπόλοιπες παραμέτρους σε όσο μικρότερες και φθηνότερες επιλογές υπάρχουν.


* speclibm: Καλύτερη επιλογή βρίσκουμε στο speclibm_4
Το κόστος υπολογίσθηκε 1760 και το Score ισούται με 4535,525. Εδώ διατηρούμε το μέγεθος της cacheline τουλάχιστον στα 128B και τις υπόλοιπες παραμέτρους σε όσο μικρότερες και φθηνότερες επιλογές υπάρχουν.

Περισσότερα δεδομένα για τα κόστη και στα σκορ, μπορείτε να βρείτε στο αρχείο [cost.xlsx](https://github.com/aikapoul/Computer-Architecture/blob/main/PART2/cost.xlsx)
